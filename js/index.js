        ...
        var log = {},
          mod = {
            face: function (editor, self) { //插入表情
              ...
              // var expressionhtml='';
              // $.ajax({
              //   url:"http://127.0.0.1:8081/expressionController/queryExpressionClass.do",
              //   async: false,
              //   success:function(data){
              //     facedata = jQuery.parseJSON(data);

              //     for(var i in facedata.expressionClassList){
              //       expressionhtml += '<li id="'+facedata.expressionClassList[i].id+'" title="' + facedata.expressionClassList[i].typeName + '">'+facedata.expressionClassList[i].typeName+'</li>'
              //     }
              //     expressionhtml = '<div><ul id="LAY-editface" class="layui-clear">' + expressionhtml + '</ul></div>';
              //   }
              // });
              // for (var key in face) {
              //   str += '<li title="' + key + '"><img src="' + face[key] + '"></li>';
              // }
              // str = '<div><ul id="LAY-editface" class="layui-clear">' + str + '</div></ul>';
              // str = '<div>'+ expressionhtml + str+'</div>';
              // console.log(str);
              // layer.tips(str, self, {
              //   tips: 3,
              //   time: 0,
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data.topicClassList, function (index, topicClass) {
        html += "<li class=\"category\"><a data-category=\"" + topicClass.id + "\">" + topicClass.typeName + "</a></li>";
      })
      $('#topic_category li:eq(0)').after(html);
    },
    error: function (data) {
      if (data.status == 403) {
...
function getUserInfo(userid) {
  $.ajax({
    url: "http://127.0.0.1:8081/userController/queryIndexUserInfoById.do",
    type: "get",
    data: {
      userId: userid
    },
    ...
    success: function (data) {
      ...
      if (data.headPic) {
        ...
        $('#index_userHeadPic').attr('src', data.headPic);
      }
      $('#userHomePic').attr('href', 'home.html?userId=' + userid);
      $('#userHomeName').attr('href', 'home.html?userId=' + userid);
      $('#head_username').html(data.userName);
      $('#index_username').prepend(data.userName);
...
function getTopicList(userid, topicClassId, range, order, page, size) {

  $.ajax({
    url: "http://127.0.0.1:8081/topicController/queryIndexTopic.do",
    type: "get",
    data: {
      userId: userid,
  ...
  var count = getTopicMore(userid, topicClassId, range, order, page + 1, size);
  if (count == 0) {
...
function getTopicMore(userid, topicClassId, range, order, page, size) {
  var count = 0;
  $.ajax({
    url: "http://127.0.0.1:8081/topicController/queryIndexTopic.do",
    type: "get",
...
function queryTopicSeeRank() {
  $.ajax({
    url: "http://127.0.0.1:8081/topicController/queryTopicSeeRank.do",
    type: "get",
    xhrFields: {
      withCredentials: true
    },
    ...
    success: function (data) {
      data = jQuery.parseJSON(data);
      var html = "";
      if (data.topicRankList.length == 0) {
        $('#see-none').attr('style', '');
      } else {
        $.each(data.topicRankList, function (index, topic) {
          html += "<dd><a href='detail.html?topicId=" + topic.id + "'>" + topic.title + "</a>";
          html += "<span style='float:right'><i class='iconfont icon-liulanyanjing'></i>&nbsp;" + topic.count + "</span></dd>";
        });
        $('#see_rank').after(html);
      }
    },
...
function queryTopicCommentRank() {
  $.ajax({
    url: "http://127.0.0.1:8081/topicController/queryTopicCommentRank.do",
    type: "get",
    xhrFields: {
      withCredentials: true
    },
    ...
    success: function (data) {
      ...
      var html = "";
      if (data.topicRankList.length == 0) {
        $('#comment-none').attr('style', '');
      } else {
        $.each(data.topicRankList, function (index, topic) {
          html += "<dd><a href='detail.html?topicId=" + topic.id + "'>" + topic.title + "</a>";
          html += "<span style='float:right'><i class='iconfont icon-pinglun1'></i>&nbsp;" + topic.count + "</span></dd>";
        });
        $('#comment_rank').after(html);
      }
    },
    error: function (data) {
      if (data.status == 403) {
