...
(function ($) {	
	...
	function require(options){
		...
		this.rules={			
			...
			isAccountExisted: function(value, errorMsg){
				errorMsg=errorMsg||" ";	
				var flag = 0;
				$.ajaxSettings.async = false;
				$.getJSON("http://127.0.0.1:8081/userController/isExistedAccount.do?account="+value, function(data){
					if(data.result == "1"){
						flag = 1;
						return errorMsg;
					}else{
			...
			isUnameExisted: function(value, errorMsg){
				errorMsg=errorMsg||" ";	
				var flag = 0;
				$.ajaxSettings.async = false;
				$.getJSON("http://127.0.0.1:8081/userController/isUnameExisted.do?userName="+value, function(data){
					if(data.result > 0){
						flag = 1;
						return errorMsg;
					}else{
            ...
            isPhoneOrEmail: function(value, errorMsg){
                errorMsg=errorMsg||" ";	
                if(!rule.phone.test(value) && !rule.email.test(value)) return errorMsg;
			},
			isPwd: function(value, errorMsg){
				errorMsg=errorMsg||" ";	
                if(!rule.passwordRegx.test(value)) return errorMsg;
            },
	...
	require.prototype={
		_init:function(config){
			...
			$('body').on({
				blur:function(event){								
					...
					if(config.phone && $(this).attr("id")==="account") $this._change($(this));	
		...
		_sendVerify:function(){	
			...
			var that=this;	
			var phoneRegex = /^1([358][0-9]|4[579]|66|7[0135678]|9[89])[0-9]{8}$/;
			...
			if(timerC === 60){
				...
				if(phoneRegex.test(account) == true){
					$.getJSON("http://127.0.0.1:8081/sendMessageController/sendMessage.do?account="+account+"&type=1&op=1", function(data){
						if(data.result == "OK"){
							document.getElementById("sendInfo").innerHTML = "请查收手机短信，并填写短信中的验证码（此验证码5分钟内有效)";
							$("#sendResult").html("短信已发送至您手机，请输入短信中的验证码，确保您的手机号真实有效。");
						}else{
							$("#sendResult").html("短信发送失败!");
						}
					});

				}else if(emailRegex.test(account) == true){
					$.getJSON("http://127.0.0.1:8081/sendMessageController/sendMessage.do?account="+account+"&type=2&op=1", function(data){
						document.getElementById("sendInfo").innerHTML = "请查收邮件，并填写邮件中的验证码（此验证码5分钟内有效)";
						$("#sendResult").html("邮件已发送至您的邮箱，请输入邮件中的验证码，确保您的邮箱真实有效。");
					});
				}
			}		
			if(timerC===0){
