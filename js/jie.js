  ...
  gather.jieAdmin = {
    ...
    del: function (div) {
        layer.confirm('确认删除该帖子么？', function (index) {
          ...
          $.ajax({
            url: "http://127.0.0.1:8081/topicController/delete.do",
            type: "get",
            data: {
              id: div.data('id')
            },
    ...
    collect: function (div) {
      var othis = $(this);
      var type = othis.attr('data-isCollected')
      if (!type) {
        type = 0;
      }

      $.ajax({
        url: "http://127.0.0.1:8081/collectController/changeCollect.do",
        type: "get",
        data: {
          topicId: div.data('id'),
          userId: userId,
        ...
        success: function (data) {
          var res = jQuery.parseJSON(data);
          if (type == 0) {
            othis.attr('data-isCollected', 1).html('取消收藏').addClass('layui-btn-danger');
            layer.msg("收藏成功!<br>经验值+" + res.point);
          } else {
            othis.attr('data-isCollected', 0).html('收藏').removeClass('layui-btn-danger');
            layer.msg("取消收藏成功!<br>经验值" + res.point);
          }
        },
        error: function (data) {
          if (data.status == 403) {
